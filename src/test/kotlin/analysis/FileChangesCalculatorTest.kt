...
class FileChangesCalculatorTest {
    ...
    private lateinit var calculator: FileChangesCalculator

    @BeforeEach
    fun setUp() {
        mockStorage = mock()
        calculator = FileChangesCalculator(mockStorage)
    }
    ...
    fun `when no source files then no changes`() {
        ...
        val result = calculator.calculateFileChanges(emptySet())
    ...
    fun `when source files are new then all files are added`() {
        ...
        val result = calculator.calculateFileChanges(sourceFiles)
    ...
    fun `when file content changes then file is marked as modified`() {
        ...
        val result = calculator.calculateFileChanges(newSourceFiles)
    ...
    fun `when no source files then store empty metadata`() {
        ...
        calculator.calculateFileChanges(emptySet())
    ...
    fun `when source files are new then store correct metadata`() {
        ...
        calculator.calculateFileChanges(sourceFiles)
    ...
    fun `when file content changes then store updated metadata`() {
        ...
        calculator.calculateFileChanges(newSourceFiles)
